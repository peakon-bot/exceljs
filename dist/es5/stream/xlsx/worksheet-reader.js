              ...
              if (c.f) {
                ...
                if (c.v) {
                  if (c.t === 'str') {
                    ...
                    value.result = parseInt(c.v.text, 10) !== 0;
                  } else if (c.t === 'e') {
                    value.result = { error: c.v.text };
                  } else {
                    value.result = parseFloat(c.v.text);
                    if (cell.style && utils.isDateFmt(cell.style.numFmt)) {
                      value.result = utils.excelToDate(value.result, properties.model.date1904);
                    }
                  }
                }
                cell.valueType = Enums.ValueType.Formula;
                cell.value = value;
              } else if (c.v) {
                switch (c.t) {
                  case 's':
                    cell.valueType = Enums.ValueType.String;
                  ...
                  case 'str':
                    ...
                    break;
                  case 'e':
                    cell.valueType = Enums.ValueType.Error;
                    cell.value = { error: c.v.text };
                    break;
                  case 'b':
                    cell.valueType = Enums.ValueType.Boolean;
                    cell.value = parseInt(c.v.text, 10) !== 0;
                    break;
                  default:
                    cell.valueType = Enums.ValueType.Number;
                    cell.value = parseFloat(c.v.text);
                    if (cell.style && utils.isDateFmt(cell.style.numFmt)) {
                      cell.value = utils.excelToDate(cell.value, properties.model.date1904);
                    }
                    break;
                }
              } else if (c.s) {
                cell.valueType = Enums.ValueType.Null;
              } else {
                cell.valueType = Enums.ValueType.Merge;
              }

              if (hyperlinks) {
                var hyperlink = hyperlinks[c.ref];
                if (hyperlink) {
                  cell.valueType = Enums.ValueType.Hyperlink;
